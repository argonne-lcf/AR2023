...
NWChemEx has been executed on Aurora for both single-GPU performance evaluations and demonstration runs on as many as 2000 nodes. Canonical coupled cluster singles and doubles (CCSD) methods for molecular description show significantly faster performance on Aurora than other achieved with other systems, as does the domain-based local pair natural orbital coupled-cluster method with single-, double- and perturbative triple excitations (DLPNO-CCSD).
